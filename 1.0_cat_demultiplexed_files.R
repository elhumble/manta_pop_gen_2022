...
file_names <- paste0("data/file_lists/", file_names)
...
run <- file_names %>% 
  ...
  filter(!grepl("rem", V1)) %>%
  mutate(rem = V1) %>%
  mutate(rem = gsub ("\\.fq.gz", ".rem.fq.gz", rem)) %>%
  mutate(out = V1)

# Create command

command <- glue("cat /exports/cmvm/eddie/eb/groups/ogden_grp/emily/manta_pop_gen_2020/data/out/demultiplexed/lib_{run$lib}/{run$V1} ",
                "/exports/cmvm/eddie/eb/groups/ogden_grp/emily/manta_pop_gen_2020/data/out/demultiplexed/lib_{run$lib}/{run$rem} > ", 
                "/exports/cmvm/eddie/eb/groups/ogden_grp/emily/manta_pop_gen_2020/data/out/demultiplexed/ena/{run$out}")
...
write.table(
  glue("{geo} \n\n {command}"),
  "data/scripts/9_cat_files_ena.sh",
  quote = F,
  col.names = F,
  row.names = F
)
...
scp data/scripts/9_cat_files_ena.sh ehumble@eddie.ecdf.ed.ac.uk:/exports/cmvm/eddie/eb/groups/ogden_grp/emily/manta_pop_gen_2020/9_cat_files_ena.sh
