...
#stem <- "data/treemix/alfredi/initial_runs/alfredi_1_m0"
#pop_order <- "data/meta/treemix_pop_order_alf.txt"

#resid_toplot <- plot_resid(stem, pop_order, min = -0.009, max = 0.009, cex = 1, usemax = T, wcols = "r")

plot_resid = function(stem, pop_order, min = -0.009, max = 0.009, cex = 1, usemax = T, wcols = "r", sp = sp){
	...
	print("here")
	names(toplot) = o[,1]
	toreturn <- plot_resid_internal(toplot, max = max, min = min, wcols = wcols, mse = mse, o = o, cex = cex, sp = sp)
	return(toreturn)
	#return <- toplot
}
...
plot_resid_internal = function(d, o = NA, max = 0.009, min = -0.009, cex =0.5, wcols = "em", mse = NA, sp = sp){
...
#	colors = c("red", "orange","yellow", "white", "green", "blue", "black")
#	pal = colorRampPalette(colors)
  pal <- colorRampPalette(c("#3B9AB2","white", "#EAD3BF"))
	ncol = 80
	cols = pal(ncol)
	...
	plot("NA", xlim = c(0, 1), ylim = c(0, 1), axes = F, xlab = "", ylab = "")
		...
		if (sp == "alfredi")
		{
		  ...
		  mtext(abbrev[i], side = 1, at =  i/npop-0.5/npop, las = 1, cex = cex, col = tcol)  
		}
		else{
		  abbrev <- c("PHI", "SL", "SA", "MC", "MP", "PERU", "ALF")  
		  #mtext(names(d)[i], side = 2, at = 1-i/npop+0.5/npop, las = 1, cex = cex, col = tcol)
		  #mtext(names(d)[i], side = 1, at =  i/npop-0.5/npop, las = 1, cex = cex, col = tcol)
		  mtext(abbrev[i], side = 2, at = 1-i/npop+0.5/npop, las = 1, cex = cex, col = tcol)
		  mtext(abbrev[i], side = 1, at =  i/npop-0.5/npop, las = 1, cex = cex, col = tcol)  
